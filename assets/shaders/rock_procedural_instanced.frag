...
void main()
{
    float dist = vUV.x;  // 0 at center, 1 at edge

    // Edge band
    float edgeWidth = max(vEdgeWidth, 0.05);
    float edgeFactor = smoothstep(1.0 - edgeWidth, 1.0, dist);

    // --- Surface detail ---
    vec2 noiseCoord = vLocalPos * 6.0;

    // Primary grain (domain-warped for organic look)
    ...
    // Subtle crack/vein pattern
    float veinPattern = veins(noiseCoord * 3.0);
    float veinDarken = smoothstep(0.3, 0.5, veinPattern) * 0.08;

    // Combine base color
    vec3 interior = vBaseColor + variation + fineGrain - veinDarken;

    // Edge color with slight noise variation
    float edgeNoise = noise(noiseCoord * 4.0) * 0.05;
    vec3 edge = vEdgeColor + edgeNoise;

    vec3 color = mix(interior, edge, edgeFactor);

    // Depth shading: slight darkening toward center
    float centerDarken = smoothstep(0.0, 0.35, dist) * 0.06;
    color -= centerDarken;

    // Subtle rim highlight (fake diffuse lighting from above)
    ...
    // Alpha: solid interior, soft edge for anti-aliasing
