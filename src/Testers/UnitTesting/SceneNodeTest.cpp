...
protected:
    ...
    {
        // Note: this is how it looks like a basic engine initialization...
        ...
        // Dependency registration (before all)
...
TEST_F(SceneNodeTestFixture, basic_scene)
{
    ...
    // expecting 5+1 entries in nodeIndex_ (5 nodes + root)
    // (root is tracked in nodeIndex_ too)
    auto indexSize = mainScene->nodeIndex_.size();
    EXPECT_TRUE(indexSize == 6);
}

TEST_F(SceneNodeTestFixture, basic_scene_serialization)
{
    const auto basicSceneFilePath = resources_->GetWorkingFolder() + paths::SCENE_DEFAULT + "\\scene_unit_test.json";
    { // IMPORTANT USE RAII OR THIS WON'T WORK !!
        std::ofstream ofs(basicSceneFilePath);
    ...
    {
        std::ifstream ifs(basicSceneFilePath);
        ...
        // expecting 3 children on the parent node...
        EXPECT_TRUE(parentNode->children_.size() == 3);
        // After Init/RebuildIndex, nodeIndex_ contains all nodes from the tree.
        // 5 nodes + root = 6 entries.
        readedScene->Init(*engineInstance_);
        EXPECT_TRUE(readedScene->nodeIndex_.size() == 6);
    }
}
...
TEST_F(SceneNodeTestFixture, test_bed_scene_serialization)
{
    ...
    {
        std::ofstream ofs(resources_->GetWorkingFolder() + paths::SCENE_DEFAULT + "/default_scene.json");
