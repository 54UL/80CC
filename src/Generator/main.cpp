...
int main(int argc, const char** argv)
{
    if (argc < 7)
    {
        std::cerr << "Usage: Generator <source_dir> <assets_dir> <output_dir> <project_name>"
                     " <core_lib> <core_include> [<entry_template_dir>]\n"
                     "       Use \"none\" for assets_dir to skip asset copying.\n";
        return 1;
    }
    ...
    const std::string projectName  = argv[4];
    const std::string coreLib      = argv[5];
    const std::string coreInc      = argv[6];
    const std::string templateHint = (argc >= 8) ? argv[7] : "";

    auto logFn = [](const std::string& msg) { std::cout << msg << "\n"; std::cout.flush(); };

    logFn("[80CC] === 80CC Generator ===");

    const bool ok = ettycc::build::RunGenerator(sourceDir,
         outputDir,
         projectName,
         coreLib,
         coreInc,
